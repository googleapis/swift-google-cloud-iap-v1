...
extension Clients {
  final class IdentityAwareProxyOAuthServiceLogging: IdentityAwareProxyOAuthServiceStub {
    let inner: any IdentityAwareProxyOAuthServiceStub
    let logger: Logger

    public init(_ inner: any IdentityAwareProxyOAuthServiceStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-iap-v1"
      logger[metadataKey: "gcp.client.service"] = "iap"
      logger[metadataKey: "gcp.experimental.swift.client"] = "IdentityAwareProxyOAuthService"
      self.inner = inner
      self.logger = logger
    }
